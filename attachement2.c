...
int main() {
    ...
    // Read file content
    long file_length;
    char *file_content = read_file("C:\\Users\\andre\\Downloads\\POP3_IMAP.pdf", &file_length);
    if (!file_content) {
        cleanup(ssl, ctx, sock);
        return 1;
    }
    ...
    printf("encoded_file_content end \n");
    free(file_content);
    ...
    // Send encoded file content in chunks
    int chunk_size = 512; // Adjust chunk size as needed
    for (long i = 0; i < strlen(encoded_file_content); i += chunk_size) {
        char chunk[chunk_size + 1];
        strncpy(chunk, encoded_file_content + i, chunk_size);
        chunk[chunk_size] = '\0';
